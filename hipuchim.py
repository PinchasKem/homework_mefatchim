...
O(n^2)
למעשה אפשר לייעל את הפונקציה ולמנוע ריצות מיותרות 
מאחר ולכל מספר יש סך מקסימאלי של ספרות שהוא יכול להוות היפוך להם 
לדוגמא הספרה 1 לא יכולה להוות היפוך לשום ספרה שתבוא אחריה
ולכן נכון לטעון שלכל ספרה מקסימום היפוכים שיבואו אחריה זה כערך הספרה פחות אחד
ואם נגביל את הפונקציה נקבל פונקציה שבמקרה הטוב רצה סדר גודל של 
O(n) 
כך זה ייראה
'''

def count_inversions_simple_effective(arr):
    count = 0
    for i in range(len(arr)-1):
        inner_count = 0
        for j in range(i+1, len(arr)):
